...
public class NamedParamGuestMessageDao implements GuestMessageDao {
	...
	public List<GuestMessage> select(int begin, int end) {
		...
		return template.query("select * from(select ROWNUM rnum, MESSAGE_ID, GUEST_NAME, MESSAGE, REGISTRY_DATE from (select * from GUESTBOOK order by MESSAGE_ID desc)) where rnum>=:startRowNum and rnum<=:count", paramMap, new RowMapper<GuestMessage>() {
			@Override
