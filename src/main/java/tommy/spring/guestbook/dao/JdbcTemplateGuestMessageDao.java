...
public class JdbcTemplateGuestMessageDao implements GuestMessageDao {
	...
	public int update(GuestMessage message) {
		...
		return jdbcTemplate.update("update GUESTBOOK set MESSAGE=? where MESSAGE_ID=?", new Object[] { message.getMessage(), message.getId() }, new int[] { Types.VARCHAR, Types.INTEGER });
	}
